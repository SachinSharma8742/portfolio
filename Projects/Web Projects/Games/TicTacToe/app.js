...
function checkResult (e){
    if(e.target.children.length<1){
        if(isSquare){
            isSquare = false
            e.target.innerHTML = '<h1 onclick="bubblingHandler(event)" class="text-9xl text-red-400 font-small">X</h1>'
        }
        else{
            isSquare = true
            e.target.innerHTML = '<div onclick="bubblingHandler(event)" class="border-8  border-[green] rounded-full w-[70%] h-[70%] flex justify-center items-center"></div>'
        }
    }
    
    if(isFilled()){
        if(!isCompleted()) {
            resultText.innerHTML = '<span class="text-5xl text-blue-500 text-center mt-20">Draw</span>'
    ...
    if(isCompleted()){
        !isSquare? resultText.innerHTML = '<span class="text-5xl text-red-500 text-center mt-20">Cross wins</span>' : resultText.innerHTML = '<span class="text-5xl text-green-500 text-center mt-20">Circle wins</span>'
        setTimeout(()=>{
            resultText.innerText = ''
            clearGame()
        }, 2000)
